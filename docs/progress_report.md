...
### 6. 数据集生成优化 (2025-03-26)

- **修复数据集分割比例问题**
  - 识别并解决了数据集重复分割导致的比例不协调问题
  - 实现了基于节点类型的分层分割策略，保证各类型节点分布一致
  - 重构了数据集生成流程，使得分割比例严格遵循配置值

- **.pt格式支持**
  - 添加了`save`和`load`方法，使用PyTorch的序列化机制
  - 适配PyTorch 2.6+的安全加载机制
  - 优化数据存储结构，减小文件大小并提高加载速度

- **数据增强优化**
  - 重构数据增强流程，仅对训练集应用增强处理
  - 改进负样本生成和节点类型平衡算法
  - 添加详细的性能统计和日志记录
  - 创建了完整的[数据集生成优化文档](./dataset_generation.md)
